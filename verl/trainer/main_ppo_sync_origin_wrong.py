...
from verl.utils.checkpoint.checkpoint_manager import BaseCheckpointManager, find_latest_ckpt_path, get_checkpoint_tracker_filename
...
class PPOTrainer:
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists(dataloader_local_path):
            dataloader_state_dict = torch.load(dataloader_local_path, map_location="cpu", weights_only=False)
        ...
        if os.path.exists(trainer_state_path):
            ...
            saved_global_steps = trainer_state.get("global_steps")
            if saved_global_steps is not None:
                self.global_steps = int(saved_global_steps)

            rng_state = trainer_state.get("rng")
            if rng_state is not None:
                BaseCheckpointManager.load_rng_state(rng_state)

            kl_ctrl_value = trainer_state.get("kl_ctrl_in_reward")
            if kl_ctrl_value is not None and hasattr(self, "kl_ctrl_in_reward"):
                self.kl_ctrl_in_reward.value = kl_ctrl_value
        else:
            logger.info(f"No trainer state found at {trainer_state_path}, skip restoring trainer-side RNG state")

    def _save_checkpoint(self):
        from verl.utils.fs import local_mkdir_safe

        local_root_dir = local_mkdir_safe(self.config.trainer.default_local_dir)
        local_global_step_folder = os.path.join(local_root_dir, f"global_step_{self.global_steps}")
        ...
        if hasattr(self, "kl_ctrl_in_reward"):
            trainer_state["kl_ctrl_in_reward"] = self.kl_ctrl_in_reward.value
        trainer_state_path = os.path.join(local_global_step_folder, "trainer_state.pt")
        torch.save(trainer_state, trainer_state_path)

        actor_checkpoint_cfg = self.config.actor_rollout_ref.actor.checkpoint
        async_save_enabled = actor_checkpoint_cfg.get("async_save", False)
        if async_save_enabled:
            logger.info("Skip writing latest_checkpointed_iteration.txt because async_save is enabled")
            return

        tracker_path = get_checkpoint_tracker_filename(local_root_dir)
        temp_tracker_path = f"{tracker_path}.tmp"
        with open(temp_tracker_path, "w") as f:
            f.write(str(self.global_steps))
        os.replace(temp_tracker_path, tracker_path)
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path):
        """Dump validation samples as JSONL."""
    ...
    def _clone_kv_batch_meta(self, batch: KVBatchMeta) -> KVBatchMeta:
        return KVBatchMeta(
            keys=list(batch.keys),
            tags=[tag.copy() if isinstance(tag, dict) else tag for tag in batch.tags],
            partition_id=batch.partition_id,
            fields=list(batch.fields) if batch.fields is not None else None,
            extra_info=dict(batch.extra_info) if batch.extra_info is not None else None,
        )
    ...
    def _get_balance_padding_multiple(self, dp_size: int) -> int:
        multiples = [dp_size]

        actor_mini_batch_size = self.config.actor_rollout_ref.actor.ppo_mini_batch_size
        if actor_mini_batch_size:
            multiples.append(actor_mini_batch_size * self.config.actor_rollout_ref.rollout.n)

        if self.use_critic:
            critic_mini_batch_size = self.config.critic.ppo_mini_batch_size
            if critic_mini_batch_size:
                multiples.append(critic_mini_batch_size * self.config.actor_rollout_ref.rollout.n)
    ...
    def _pad_batch_for_training(
        ...
        padded_batch = self._clone_kv_batch_meta(batch)
        original_batch_size = len(padded_batch)
        if original_batch_size == 0:
            ...
            metrics[f"{logging_prefix}/padded_batch_size"] = 0
            return padded_batch
        ...
        if padding_size == 0:
            padded_batch.extra_info["padding_keys"] = []
            return padded_batch

        padding_indices = [(original_batch_size - 1 - (i % original_batch_size)) for i in range(padding_size)]
        padding_indices.reverse()
        padding_source_keys = [batch.keys[i] for i in padding_indices]
        padding_keys = [f"{source_key}__pad_{uuid.uuid4().hex}" for source_key in padding_source_keys]
        padding_tags = []
        for padding_pos, idx in enumerate(padding_indices):
            source_tag = batch.tags[idx] if idx < len(batch.tags) else {}
            padding_tag = source_tag.copy() if isinstance(source_tag, dict) else {}
            padding_tag["is_padding"] = True
            padding_tag["padding_position"] = padding_pos
            padding_tag["padding_source_key"] = batch.keys[idx]
            padding_tags.append(padding_tag)

        padding_fields = tq.kv_batch_get(
            keys=padding_source_keys,
            partition_id=batch.partition_id,
            fields=batch.fields,
        )
        tq.kv_batch_put(
        ...
        combined_fields = list(batch.fields) if batch.fields is not None else list(padding_fields.keys())
        padded_batch.keys.extend(padding_keys)
        padded_batch.tags.extend(padding_tags)
        padded_batch.fields = combined_fields
        padded_batch.extra_info["padding_keys"] = padding_keys
        object.__setattr__(padded_batch, "_size", len(padded_batch.keys))
        return padded_batch
    ...
    def _balance_batch(self, batch: KVBatchMeta, metrics, logging_prefix="global_seqlen", keep_minibatch=False):
        ...
        target_multiple = self._get_balance_padding_multiple(dp_size)
        train_batch = self._pad_batch_for_training(batch, target_multiple, metrics, logging_prefix)

        global_seqlen_lst = torch.tensor([tag["seq_len"] for tag in train_batch.tags], dtype=torch.int64)
        workload_lst = calculate_workload(global_seqlen_lst)

        global_partition_lst = get_seqlen_balanced_partitions(workload_lst, k_partitions=dp_size, equal_size=True)
        train_batch.reorder([j for partition in global_partition_lst for j in partition])
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                tq.kv_clear(keys=batch.keys, partition_id=batch.partition_id)
                padding_keys = batch.extra_info.get("padding_keys", [])
                if padding_keys:
    ...
    def step(self, batch_dict: dict, metrics: dict, timing_raw: dict) -> KVBatchMeta:
        ...
        if need_train_batch:
            # 8. balance and pad a dedicated training batch for actor/critic updates
            train_batch = self._balance_batch(batch, metrics=metrics)
            batch.extra_info["padding_keys"] = list(train_batch.extra_info.get("padding_keys", []))

        # 9. [OPTIONAL] update critic
        if self.use_critic:
            with marked_timer("update_critic", timing_raw, color="pink"):
                assert train_batch is not None
                train_batch = self._update_critic(train_batch, metrics=metrics)

        # 10. update actor
        if self.config.trainer.critic_warmup <= self.global_steps:
            with marked_timer("update_actor", timing_raw, color="red"):
                assert train_batch is not None
                train_batch = self._update_actor(train_batch, metrics=metrics)
