...
def _normalize_token_ids(tokenized_prompt: Any) -> list[int]:
    ...
    if tokenized_prompt and isinstance(tokenized_prompt[0], list):
        if len(tokenized_prompt) != 1:
            raise ValueError("Expected a single tokenized prompt sequence.")
        tokenized_prompt = tokenized_prompt[0]
    return [int(token_id) for token_id in tokenized_prompt]


def _initialize_system_prompt(tokenizer: Any, **apply_chat_template_kwargs) -> list[int]:
    ...
    return _normalize_token_ids(tokenized_prompt)
...
class SummarizerContextManager(ContextManager):
    ...
    def __init__(
        ...
        *,
        ...
        self.system_prompt_length = len(_initialize_system_prompt(self.tokenizer, **self.apply_chat_template_kwargs))
    ...
    async def _compress_impl(self, state: ContextState) -> ContextState:
        ...
        # We use chat_template to rebuild summary ids because we need add_generation_prompt=True
        # to encourage continued generation after the </summary> tag. We keep the original
        # prompt_ids unchanged since it may already encode multimodal prompt state.
        tokenized_summary = self.tokenizer.apply_chat_template(
            [{"role": "assistant", "content": summary_text}],
            add_generation_prompt=True,
            tokenize=True,
            **self.apply_chat_template_kwargs,
        )
        summary_ids = _normalize_token_ids(tokenized_summary)[self.system_prompt_length :]
